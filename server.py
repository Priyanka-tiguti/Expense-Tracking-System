...
def get_analytics_category(date_range:DateRange):
    ...
    if data is None:
        return HTTPException(status_code=500, detail="Failed to retrieve expense summary from the database.")
...
def get_analytics_by_months():
    ...
    if monthly_summary is None:
        return HTTPException(status_code=500, detail="Failed to retrieve monthly expense summary from the database.")
